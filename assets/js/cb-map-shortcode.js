...
function CB_Map() {
  ...
  cb_map.get_location_data = function(init) {
    ...
    jQuery.get(this.settings.data_url + 'items', data, function(response) {
      cb_map.location_data = cb_map.parse_api_data(response);
  ...
  cb_map.parse_api_data = function(data) {
    var items = {};
    var locations = {};

    var input_items = data.items;
    var input_locations = data.locations;
    var input_availability = data.availability;
    console.log('input_locations: ', input_locations);

    input_items.forEach((input_item, i) => {
      items[input_item.id] = {
    ...
    input_locations.forEach((input_location) => {
      if(input_location.geometry) {
        var location = {
          ...
          name: input_location.properties.name,
          address: input_location.properties.address,
          lat: input_location.geometry.coordinates[1],
          lon: input_location.geometry.coordinates[0],
          items: {}
        }
