...
class CB_Map_Shortcode {
      ...
      if($post && $post->post_type == 'cb_map') {
        ...
        if($post->post_status == 'publish') {
          if($map_type == 1 || $map_type == 2) {
            ...
            return '<div>' . cb_map\__( 'NO_VALID_MAP_TYPE', 'commonsbooking-maps', 'no valid map type') . '</div>';
          }
        }
        else {
          return '<div>' . cb_map\__( 'NO_VALID_POST_STATUS', 'commonsbooking-maps', 'map is not published') . '</div>';
        }
      }
      else {
        return '<div>' . cb_map\__( 'NO_VALID_MAP_ID', 'commonsbooking-maps', 'no valid map id provided') . '</div>';
      }
  ...
  **/
  public static function get_settings($cb_map_id) {
    ...
    $max_days_in_future = 30; //$commons_booking_settings['commons-booking_bookingsettings_daystoshow']; TODO: CB2 replacement?
    ...
    $settings = [
      'data_url' => get_rest_url(null, 'commonsbooking/v1/'), //get_site_url(null, '', null) . '/wp-admin/admin-ajax.php',
  ...
  **/
  public static function get_translation($cb_map_id) {
    ...
    $translation = [
      'OPENING_HOURS' => strlen($label_location_opening_hours) > 0 ? $label_location_opening_hours : cb_map\__('OPENING_HOURS', 'commonsbooking-maps', 'opening hours'),
      'CONTACT' => strlen($label_location_contact) > 0 ? $label_location_contact : cb_map\__('CONTACT', 'commonsbooking-maps', 'contact'),
      'FROM' => cb_map\__( 'FROM', 'commonsbooking-maps', 'from'),
      'UNTIL' => cb_map\__( 'UNTIL', 'commonsbooking-maps', 'until'),
      'AT_LEAST' => cb_map\__( 'AT_LEAST', 'commonsbooking-maps', 'for at least'),
      'DAYS' => cb_map\__( 'DAYS', 'commonsbooking-maps', 'day(s)'),
      'NO_LOCATIONS_MESSAGE' => strlen($custom_no_locations_message) > 0 ? $custom_no_locations_message : cb_map\__( 'NO_LOCATIONS_MESSAGE', 'commonsbooking-maps', 'Sorry, no locations found.'),
      'FILTER' => cb_map\__( 'FILTER', 'commonsbooking-maps', 'filter'),
      'AVAILABILITY' => strlen($label_item_availability_filter) > 0 ? $label_item_availability_filter : cb_map\__( 'AVAILABILITY', 'commonsbooking-maps', 'availability'),
      'CATEGORIES' => strlen($label_item_category_filter) > 0 ? $label_item_category_filter : cb_map\__( 'CATEGORIES', 'commonsbooking-maps', 'categories'),
      'DISTANCE' => strlen($label_location_distance_filter) > 0 ? $label_location_distance_filter : cb_map\__( 'DISTANCE', 'commonsbooking-maps', 'distance'),
      'ADDRESS' => cb_map\__( 'ADDRESS', 'commonsbooking-maps', 'address'),
      'GEO_SEARCH_ERROR' => cb_map\__( 'GEO_SEARCH_ERROR', 'commonsbooking-maps', 'Sorry, an error occured during your request. Please try again later.'),
      'GEO_SEARCH_UNAVAILABLE' => cb_map\__( 'GEO_SEARCH_UNAVAILABLE', 'commonsbooking-maps', 'The service is currently not available. Please try again later.'),
      'COMING_SOON' => cb_map\__( 'COMING_SOON', 'commonsbooking-maps', 'comming soon')
    ];
